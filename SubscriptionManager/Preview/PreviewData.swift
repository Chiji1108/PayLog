...
enum PreviewData {
    ...
    private static func seedSampleData(in context: ModelContext) {
        ...
        let suica = ElectronicMoney(name: "Suica", notes: "通勤用に毎月チャージしています。", card: visa, createdAt: now.addingTimeInterval(-60 * 60 * 24 * 2))
        let payPay = ElectronicMoney(name: "PayPay", card: master, isActive: false, createdAt: now.addingTimeInterval(-60 * 60 * 24))
